...
class Numgle {
  ...
  Numgle(String str) {
    str = str;
    dataset =
        (jsonDecode(datasetFile.readAsStringSync()) as Map<String, dynamic>);
  ...
  String convertStringToNumgle(String input) {
    ...
    input.runes.forEach((element) {
      arr.add(convertCharToNumgle(new String.fromCharCode(element)));
    });
  ...
  String completeHangul(String input) {
    ...
    return dataset["jong"][separateHan1["jong"]!] +
        dataset["cj"][Math.min(8, separateHan1["jung"]!)]
            [separateHan1["cho"]!] +
        dataset["cho"][separateHan1["cho"]!];
  }
